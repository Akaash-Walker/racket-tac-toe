...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname Racket-Tac-Toe) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
(require 2htdp/image)
...
(define (render ws)
  (cond
    [(= PX_WIN (ws-gs ws))
     (overlay
      (text "X Wins!" END_TEXT_SIZE "green")
      (draw-moves BOARD (ws-bd ws))
      MTS)]
    [(= PO_WIN (ws-gs ws))

     (overlay
      (text "O Wins!" END_TEXT_SIZE "red")
      (draw-moves BOARD (ws-bd ws))
      MTS)]
    [(= DRAW (ws-gs ws))
     (overlay
      (text "Draw :(" END_TEXT_SIZE "orange")
      (draw-moves BOARD (ws-bd ws))
      MTS)]
    [else
     (overlay
